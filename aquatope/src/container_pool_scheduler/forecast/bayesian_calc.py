...
from typing import Callable, Dict, Iterable, List, Mapping, Optional, Sequence, Tuple, Union, Any
...
class AreaState:
    """
    Online Bayesian calibration state for one region using Normal-Inverse-Gamma.

    Residual model (relative to a chosen central forecast):
        r = Y - center_forecast
        r | b, sigma2 ~ Normal(b, sigma2)

    Conjugate prior / posterior (Normal-Inverse-Gamma):
        sigma2 ~ InvGamma(alpha, beta)          (shape alpha, scale beta)
        b | sigma2 ~ Normal(mu, sigma2 / kappa)

    Parameters:
      - mu: posterior location for bias mean
      - kappa: posterior strength for bias mean (like "effective prior observations")
      - alpha, beta: posterior parameters for sigma2 (InvGamma)

    Notes:
      - Posterior mean of sigma2 exists only if alpha > 1.
      - This replaces EWMA sigma2 with a fully Bayesian update.
    """
    mu: float = 0.0
    kappa: float = 1.0
    alpha: float = 2.0
    beta: float = 50.0  # scale; prior mean sigma2 = beta / (alpha - 1) if alpha > 1
    ...
    @property
    def bias_var_marginal(self) -> float:
        """
        Marginal posterior variance of b (integrating out sigma2).
        Var(b | data) = E[sigma2] / kappa, where E[sigma2]=beta/(alpha-1) if alpha>1.
        """
        if self.alpha <= 1.0:
            return float("inf")
        return float((self.beta / (self.alpha - 1.0)) / max(self.kappa, 1e-12))
...
def make_default_state(
    prior_rms_error: float = 5.0,
    bias_pseudo_obs: float = 1.0,
    var_confidence: float = 2.0,
) -> AreaState:
    """
    prior_rms_error: expected |residual| in same units as y
    bias_pseudo_obs: effective observations for bias mean
    var_confidence: alpha parameter (>= 2 recommended)
    """
    alpha = max(var_confidence, 1.5)
    beta = (alpha - 1.0) * (prior_rms_error ** 2)

    return AreaState(
        mu=0.0,
        kappa=bias_pseudo_obs,
        alpha=alpha,
        beta=beta,
    )
...
class BiasCalibratedThompsonCVaRRouter:
    """
    Combines:
      (1) Online Bayesian bias + variance calibration per region (Normal-Inverse-Gamma)
      (2) Thompson sampling using MC predictive samples + (b, sigma2) posterior + switching cost
    ...
    Where:
      ...
      (sigma2*_i, b*_i) are random draws from NIG posterior (Thompson)
      calibrated_dist_i approximates MC samples + bias uncertainty (+ optional observation noise)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Discounting for non-stationarity (0,1], 1.0 = no discount
        discount: float = 1.0,
        ...
        cvar_num_posterior_draws: int = 30,
        cvar_max_mc_used: int = 300,
        include_obs_noise_in_cvar: bool = True,
        # MC sampling control:
        ...
        #     r: AreaState(mu=prior_mu, kappa=prior_kappa, alpha=prior_alpha, beta=prior_beta)
        #     for r in self.regions
        # }

        self.state = {
            r: make_default_state(prior_rms_error=5.0)
            for r in self.regions
        }
        ...
        self.cvar_num_posterior_draws = int(cvar_num_posterior_draws)
        self.cvar_max_mc_used = int(cvar_max_mc_used)
        self.include_obs_noise_in_cvar = bool(include_obs_noise_in_cvar)
    ...
    def decide(
        ...
        *,
        ...
        for r in self.regions:
            ...
            # Thompson draw from posterior: sigma2 then b | sigma2
            sigma2_star = self._draw_sigma2(st)
            b_star = float(self.rng.normal(loc=st.mu, scale=np.sqrt(sigma2_star / max(st.kappa, 1e-12))))
            ...
            diagnostics[r] = {
                ...
                "beta": float(st.beta),
                "sigma2_mean": float(st.sigma2_mean),
                "bias_var_marginal": float(st.bias_var_marginal),
            }
    ...
    def update(
        ...
        *,
        ...
        """
        Update the NIG posterior using observed actuals.
        ...
        for r, y in observed_actual.items():
            ...
            # Discounting (forgetting) for non-stationarity:
            # Heuristic: shrink "effective sample size" parameters toward weak-prior.
            if self.discount < 1.0:
                d = self.discount
                st.kappa = 1.0 + (st.kappa - 1.0) * d
                st.alpha = 1.0 + (st.alpha - 1.0) * d
                st.beta = st.beta * d

            mu0, kappa0, alpha0, beta0 = st.mu, st.kappa, st.alpha, st.beta
            rres = float(y - center[r])

            # Conjugate NIG update with one observation rres
            kappa1 = kappa0 + 1.0
            mu1 = (kappa0 * mu0 + rres) / kappa1
            alpha1 = alpha0 + 0.5
            beta1 = beta0 + 0.5 * (kappa0 * (rres - mu0) ** 2) / kappa1

            st.mu = float(mu1)
            st.kappa = float(max(kappa1, 1e-12))
            st.alpha = float(max(alpha1, 1e-12))
            st.beta = float(max(beta1, 1e-12))
            ...
            updates[r] = {
                ...
                "residual": float(rres),
                "mu_old": float(mu0),
                "kappa_old": float(kappa0),
                "alpha_old": float(alpha0),
                "beta_old": float(beta0),
                "mu_new": float(st.mu),
                "kappa_new": float(st.kappa),
                "alpha_new": float(st.alpha),
                "beta_new": float(st.beta),
                "sigma2_mean_new": float(st.sigma2_mean),
            }
    ...
    def _draw_sigma2(self, st: AreaState) -> float:
        """
        Draw sigma2 ~ InvGamma(alpha, beta).
        ...
        beta = max(st.beta, 1e-12)
        g = self.rng.gamma(shape=alpha, scale=1.0 / beta)
        return float(1.0 / max(g, 1e-300))
    ...
    def _calibrated_samples_for_cvar(self, X: np.ndarray, st: AreaState) -> np.ndarray:
        ...
        K = max(1, int(self.cvar_num_posterior_draws))

        # Draw K posterior samples of (sigma2, b)
        sigma2_draws = np.array([self._draw_sigma2(st) for _ in range(K)], dtype=float)
        b_draws = self.rng.normal(
            loc=st.mu,
            scale=np.sqrt(sigma2_draws / max(st.kappa, 1e-12)),
            size=K,
        )

        # Combine with MC predictive samples (center distribution)
        base = b_draws.reshape(-1, 1) + X_use.reshape(1, -1)

        # Optionally include observation noise eps ~ N(0, sigma2)
        if self.include_obs_noise_in_cvar:
            eps = self.rng.normal(loc=0.0, scale=np.sqrt(sigma2_draws).reshape(-1, 1), size=base.shape)
            base = base + eps
